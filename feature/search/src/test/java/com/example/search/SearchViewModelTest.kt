...
class SearchViewModelTest {
    ...
    fun success_data_searchList() {
        ...
        } returns TheMovieDBResult.Success(TestData.testSearchResponse)
        // act
        viewModel.refresh()
        // assert
        assert(observerStatus.get() == TheMovieDBStatus.ReLoading)
        observerList.await()
        assertThat(observerList.get(), `is`(TestData.testSearchResponse.results))
        assertThat(observerStatus.get(), `is`(TheMovieDBStatus.Success))
    }
}
